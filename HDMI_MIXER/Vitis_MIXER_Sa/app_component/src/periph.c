...
void XPeriph_ConfigTpg(XPeriph *InstancePtr)
{
  ...
  if (InstancePtr->TpgConfig.IsInterlaced)
    XV_tpg_Set_height(pTpg, InstancePtr->TpgConfig.Height);//def div by 2
  else
    XV_tpg_Set_height(pTpg, InstancePtr->TpgConfig.Height*2);//interlaced
  ...
  if (InstancePtr->TpgConfig.IsInterlaced) {
    ...
    //XV_tpg_Set_Polarity(pTpg, 1);//set "fid" polarity (0-def)
  }
  else{
    XV_tpg_Set_Interlaced(pTpg, 0);
  }

  if(pTpg->Config.ForegroundEnable) {
...
void XPeriph_TpgDbgReportStatus(XPeriph *InstancePtr)
{
  ...
  if(pTpg)
  {
	...
	xil_printf("Interlace: %d\r\n",InstancePtr->TpgConfig.IsInterlaced);
	xil_printf("---------\r\n");
  }
}
 ...
 *
 *****************************************************************************/
void XPeriph_ConfigVtc(XVtc *VtcPtr,
...
//  XVtc_SetSource(InstancePtr->VtcPtr, &SourceSelect);
  ...
  XVtc_WriteReg(VtcPtr->Config.BaseAddress, (XVTC_CTL_OFFSET), 0x07FFFF00);
  ...
  XVtc_SetGeneratorTiming(VtcPtr, &VideoTiming);
