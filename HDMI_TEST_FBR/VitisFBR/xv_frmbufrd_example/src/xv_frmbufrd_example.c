...
#define DDR_BASE_ADDRESS XPAR_DDR_MEM_BASEADDR // Defined in xparameters.h
...
int DDR_Write(void)
{
    ...
    for(i=0;i<1;i++)
    {
      // Method 2: Using Xilinx I/O functions
      //Xil_Out32(XVFRMBUFRD_BUFFER_BASEADDR + i, 0x00FF00FF);
      //value = Xil_In32(XVFRMBUFRD_BUFFER_BASEADDR + i);
      ///xil_printf("Value written to DDR at %x: %x\\n\\r", (unsigned int)(DDR_BASE_ADDRESS + i), value);
      //xil_printf("%x: %x\n\r", (unsigned int)(XVFRMBUFRD_BUFFER_BASEADDR + i), value);
      //return 0;      
    }
}
