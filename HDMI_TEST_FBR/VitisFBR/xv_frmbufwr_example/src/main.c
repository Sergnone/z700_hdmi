...
void TPG_videoIpConfig(XVidC_VideoMode videoMode)
{
...
int TPG_videoClockConfig(XVidC_VideoMode videoMode)
{
	u32 DIVCLK_DIVIDE = 4;
                        ...
                        XVidC_VideoStream *StreamPtr)
{
  ...
  XVFrmbufWr_InterruptDisable(&frmbufwr, XVFRMBUFRD_IRQ_DONE_MASK);
...
Status = XSetupInterruptSystem(&frmbufrd,&XVFrmbufRd_InterruptHandler,
  ...
  */
  ...
  //}
	...
	Status = TPG_videoClockConfig(TestMode);
	if(Status != XST_SUCCESS) {
		return(XST_FAILURE);
	}
	TPG_videoIpConfig(TestMode);
