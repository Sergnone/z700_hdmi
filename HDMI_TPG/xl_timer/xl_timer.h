...
#include "xl_timer_config.h"
#include "xl_io.h"
//#include "xil_printf.h"
#include "xl_status.h"
#include "xl_bspconfig.h"
#include "xl_util.h"
...
typedef void (*XL_Timer_TickHandler) (void *CallBackRef, uint32_t StatusEvent);
 ...
 */
typedef struct XL_TimerTag {
	void (*XL_Timer_ModifyInterval)(struct XL_TimerTag *InstancePtr, uint32_t delay,
               XL_Timer_DelayType Delaytype); /**<Modifies the timer interval*/
	void (*XL_Timer_TickIntrHandler)(struct XL_TimerTag *InstancePtr,
               uint8_t Priority);                /**< Tick interrupt handler */
	void (*XL_Timer_TickInterval)(struct XL_TimerTag *InstancePtr, uint32_t Delay);
                                            /**< Configures the tick interval */
	void (*XL_SleepTimer_Stop)(struct XL_TimerTag *InstancePtr);
                                            /**< Stops the sleep timer */
	void (*XL_TickTimer_Stop)(struct XL_TimerTag *InstancePtr);
                                            /**< Stops the tick timer */
	void (*XL_TickTimer_ClearInterrupt)(struct XL_TimerTag *InstancePtr);
	                                    /**< Clears the Tick timer interrupt status */
	XL_Timer_TickHandler Handler;         /**< Callback function */
	void *CallBackRef;                  /**< Callback reference for handler */
#ifdef  XPM_SUPPORT
	void (*XL_TickTimer_ReleaseTickTimer)(struct XL_TimerTag *InstancePtr);
...
typedef uint64_t XL_Time;
extern XL_Timer TimerInst;
...
uint32_t XL_Timer_GetLibVersion(void)
{
 ...
 */
uint32_t XlSleepTimer_Init(XL_Timer *InstancePtr);
/**
 * This API is used for initializing Tick timer
 */
uint32_t XlTickTimer_Init(XL_Timer *InstancePtr);
/**
 * Get the time
 */
void XL_Time_GetTime(XL_Time *Xtime_Global);
...
uint32_t Xl_GetMBFrequency(void);
uint32_t Xl_SetMBFrequency(uint32_t Val);
#endif

#ifdef XTIMER_DEFAULT_TIMER_IS_MB_RISCV
uint32_t Xl_GetRISCVFrequency(void);
uint32_t Xl_SetRISCVFrequency(uint32_t Val);
#endif
