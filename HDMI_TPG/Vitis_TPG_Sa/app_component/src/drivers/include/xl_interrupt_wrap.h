...
extern int XConnectToFastInterruptCntrl(uint32_t IntrId, void *IntrHandler, UINTPTR IntcParent);
#endif
extern int XDisconnectInterruptCntrl(uint32_t IntrId, UINTPTR IntcParent);
extern int XStartInterruptCntrl(uint32_t Mode, UINTPTR IntcParent);
extern void XEnableIntrId( uint32_t IntrId, UINTPTR IntcParent);
extern void XDisableIntrId( uint32_t IntrId, UINTPTR IntcParent);
#if defined (XPAR_SCUGIC)
extern void XSetPriorityTriggerType( uint32_t IntrId, uint8_t Priority, UINTPTR IntcParent);
extern void XGetPriorityTriggerType( uint32_t IntrId, uint8_t *Priority, uint8_t *Trigger, UINTPTR IntcParent);
#endif
...
extern int XSetupInterruptSystem(void *DriverInstance, void *IntrHandler, uint32_t IntrId,
				 UINTPTR IntcParent,
				 uint16_t Priority);
extern s32 XGetEncodedIntrId(uint32_t LegacyIntrId, uint32_t TriggerType, uint8_t IntrType, uint8_t IntcType,
			     uint32_t *IntrId);
extern s32 XTriggerSoftwareIntr(uint32_t IntrId, UINTPTR IntcParent, uint32_t Cpu_Id);
#endif
