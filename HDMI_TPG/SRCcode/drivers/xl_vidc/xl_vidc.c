...
extern const XL_VidC_VideoTimingMode XL_VidC_VideoTimingModes[XVIDC_VM_NUM_SUPPORTED];
...
{
	if (XL_VidC_GetVideoFormat(VmId) == XVIDC_VF_INTERLACED) {
		return 1;
	}
					...
					uint8_t IsInterlaced)
{
	...
	if (ResFound) {
		...
		Mode =
		(ResFound) ? (XL_VidC_VideoMode)Mid : (XVIDC_VM_NOT_SUPPORTED);
	}
	else {
